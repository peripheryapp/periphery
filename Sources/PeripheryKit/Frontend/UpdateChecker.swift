...
class UpdateChecker: Singleton {
    ...
    func run() {
        ...
        let task = urlSession.dataTask(with: urlRequest) { [weak self] data, _, error in
            ...
            if let error = error {
                strongSelf.logger.debug("[update:check] ERROR: \(error.localizedDescription)")
            ...
            guard let jsonData = data,
                ...
                let tagName = jsonObject["tag_name"] as? String else {
                    ...
                    strongSelf.error = PeripheryKitError.updateCheckError(message: message)
                    strongSelf.logger.debug("[update:check] \(message)")
                    strongSelf.semaphore.signal()
                    return
            }
    ...
    func notifyIfAvailable() {
        ...
        logger.debug("[update-checker] latest: \(latestVersion)")
    ...
    func wait() -> Result<String, PeripheryKitError> {
        ...
        if waitResult == .timedOut {
            return .failure(PeripheryKitError.updateCheckError(message: "Timed out while checking for update."))
        }
        ...
        return .failure(PeripheryKitError.updateCheckError(message: "Failed to determine latest version."))
    }
}
