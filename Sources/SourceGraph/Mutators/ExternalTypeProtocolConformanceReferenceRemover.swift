...
final class ExternalTypeProtocolConformanceReferenceRemover: SourceGraphMutator {
    ...
    func mutate() throws {
        ...
        for extDecl in extensionDecls {
            ...
            let protocolDecls = extDecl.related.filter { $0.declarationKind == .protocol }.map { graph.declaration(withUsrID: $0.usrID) }
            ...
            for relatedRef in relatedRefs {
                ...
                guard
                    let relatedDecl = graph.declaration(withUsrID: relatedRef.usrID),
